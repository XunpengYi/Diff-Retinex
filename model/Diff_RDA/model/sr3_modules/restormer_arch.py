...
class Restormer(nn.Module):
    def __init__(self,
        ...
        for i in range(num_blocks[0]):
            layers.append(TransformerBlock(dim=dim, num_heads=heads[0], ffn_expansion_factor=ffn_expansion_factor, bias=bias,
        ...
        for i in range(num_blocks[1]):
            layers.append(TransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type))
            layers.append(FeatureWiseAffine(dim, int(dim * 2 ** 1)))
        ...
        for i in range(num_blocks[2]):
            layers.append(TransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], ffn_expansion_factor=ffn_expansion_factor,
                             bias=bias, LayerNorm_type=LayerNorm_type))
            if i <= num_blocks[2] - 1 and (i + 1) % 2 == 0:
                layers.append(FeatureWiseAffine(dim, int(dim * 2 ** 2)))
