...
class GaussianDiffusion(nn.Module):
    ...
    def set_new_noise_schedule(self, schedule_opt, device):
        ...
        lvlb_weights_eps = self.betas ** 2 / (2 * self.posterior_variance * to_torch(alphas) * (1 - self.alphas_cumprod))
        lvlb_weights_x0 = 0.5 * np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod))
        lvlb_weights_eps[0] = lvlb_weights_eps[1]
        lvlb_weights_x0[0] = lvlb_weights_x0[1]
        self.register_buffer('lvlb_weights_eps', lvlb_weights_eps, persistent=False)
        self.register_buffer('lvlb_weights_x0', lvlb_weights_x0, persistent=False)
        assert not torch.isnan(self.lvlb_weights_eps).all()
        assert not torch.isnan(self.lvlb_weights_x0).all()
    ...
    def unbalanced_L1_loss(self, I_pred, I_real):
        ...
        diff = I_real - I_pred
        mask = diff < 0
        loss = alpha * torch.sum(torch.abs(diff) * mask) + torch.sum(torch.abs(diff) * ~mask)
        return loss
