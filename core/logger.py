...
def parse(args):
    ...
    opt_RDA_path = args.config_RDA
    json_str = ''
    with open(opt_RDA_path, 'r') as f:
        for line in f:
            line = line.split('//')[0] + '\n'
            json_str += line
    opt_RDA = json.loads(json_str, object_pairs_hook=OrderedDict)

    opt_IDA_path = args.config_IDA
    json_str = ''
    with open(opt_IDA_path, 'r') as f:
        for line in f:
            line = line.split('//')[0] + '\n'
            json_str += line
    opt_IDA = json.loads(json_str, object_pairs_hook=OrderedDict)
